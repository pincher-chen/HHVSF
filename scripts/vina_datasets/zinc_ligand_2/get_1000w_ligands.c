      ...
      char *argv[])
{
   ...
   collection1 = mongoc_client_get_collection (client, "zinc_ligand_1000w", "zinc_data");
    ...
    index_name = (char*) malloc(sizeof(char)*512);
    pdbqt_file = (char*) malloc(sizeof(char)*1024000);


    command = BCON_NEW ("enablesharding", BCON_UTF8 ("zinc_ligand_1000w")
                        );
    command1 = BCON_NEW ( "shardcollection",BCON_UTF8("zinc_ligand_1000w.zinc_data"),
