...
#collection_name = raw_input("Please input the collection name: ")
  ...
  mol_file_dir="~/data/zinc_conformer/data_split/"+sys.argv[1]
  mongodb_addr="cn16377:16377"
  db_name="zinc_conformer_1"
...
#  collection.create_index([('idxd',pymongo.ASCENDING)],unique=True) #index within dataset
  ...
  for mol_file_name in mol_file_list:
    ...
    molfile=f.read(size).decode("unicode_escape")
    collection.insert({"idxd":count,"sdf_name":mol_file_name,"sdf_data":molfile})
    count+=1
    
#    print tmp
#    mol_file=pybel.readfile("sdf",mol_file_dir+"/"+mol_file_name)
#    print mol_file
'''
    while True:
      try:
...
#        collection.insert({"idxd":count,"index_name":tmp.title,"mol2_file":tmp.write(format="mol2"),"file_name":mol_file_name})
        collection.insert({"idxd":count,"sdf_name":mol_file_name,"sdf_data":tmp})
        if((count%5000)==0):
	  print "count=",count
        count+=1
