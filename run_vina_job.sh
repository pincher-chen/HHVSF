...
export mongo_path=/NSFCGZ/app/mongo-api/c/1.4.2/
...
#echo " db.runCommand( { enablesharding :"vina_result"})" >>$(mongo_shell_dir)/mongo_initial.js
echo " db.runCommand( { shardcollection : \"vina_result.${read_database}_${job_id}\",key : {\"score\": 1} } );" >>${mongo_shell_dir}/${job_id}_enableSharding.js
...
#ssh cn16354 /HOME/nscc-gz_vscreening_1/WORKSPACE/compute/condor/condor_vina_submit.sh $job_id $read_database $target_name $node_num 
ssh cn16370 /HOME/nscc-gz_pinchen/compute/condor/condor_vina_submit.sh $job_id $read_database $target_name $node_num 
##update start time
